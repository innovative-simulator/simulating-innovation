...
to setup-field
  ...
  repeat foundational-papers [
    create-papers 1 [
      ...
      set pdate 0
      set status 0
      set contents array:from-list n-values contents-size [random 2]
      calc-paper-fitness
    ...
    ask cur-paper [
      ...
      repeat contents-size [
        ifelse random-float 1 < chance-bit-innovative [
          array:set contents cur-bit (random 2)
        ]
        [
...
to-report generated-author [given-paper]
  ...
  ifelse (random-float 1) < chance-new-to-field [
    ...
    create-authors 1 [
      ...
      set color red
      set beliefs array:from-list n-values contents-size [random 2]
      set fitness 0
      set cur-author self
      set num-new-authors num-new-authors + 1
    ]
...
to-report authors-similarity [ego alter]
  report sum (map [ifelse-value (?1 = ?2) [1] [0]] (array:to-list [beliefs] of ego) (array:to-list [beliefs] of alter))
end

to-report author-paper-similarity [given-author given-paper]
  report sum (map [ifelse-value (?1 = ?2) [1] [0]] (array:to-list [beliefs] of given-author) (array:to-list [contents] of given-paper)) 
end

to-report papers-similarity [ego alter]
  report sum (map [ifelse-value (?1 = ?2) [1] [0]] (array:to-list [contents] of ego) (array:to-list [contents] of alter)) 
end
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
; Reference
...
;  print (word "Reviewing paper " who)
  ...
  ask out-rlink-neighbors [
    if acceptible [ 
      set acceptible (contents-size > papers-similarity self myself)
    ]
  ]
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
; Fitness function: Kauffman's NK landscapes
to setup-NK
  ; Define input nodes for each node
  set input-sets array:from-list n-values contents-size [array:from-list sublist (shuffle n-values (contents-size - 1) [?]) 0 (interdependencies + 1) ] ; New version. Guarantees K unique input nodes.
;  set input-sets array:from-list n-values contents-size [array:from-list n-values (interdependencies + 1) [random (contents-size - 1)]] ; Version used for book.
  let cur-node 0
  let cur-input 0
  let input-set 0
  repeat contents-size [
    ...
    repeat interdependencies [ ; Each node has K inputs which are not itself
  ...
  set fitness-tables array:from-list n-values contents-size [array:from-list n-values (2 ^ (interdependencies + 1)) [random-float 1]]
end

to calc-agent-fitness
  if fitness-method ="NK Fitness" [
    let fitness-sum 0
    let cur-node 0
    repeat contents-size [
      set fitness-sum fitness-sum + 
      array:item (array:item fitness-tables cur-node) (sum n-values (interdependencies + 1) [(array:item beliefs (array:item (array:item input-sets cur-node) ?)) * (2 ^ ?)])
      set cur-node cur-node + 1
    ]
    set fitness (fitness-sum / contents-size)
    ;set fitness ((fitness-sum / contents-size) / base-max-fitness) ^ 8 ; Lazer & Friedman's definition
    stop
  ]
  set fitness 1
end
...
to calc-paper-fitness
  if fitness-method ="NK Fitness" [
    ...
    repeat contents-size [
      set fitness-sum fitness-sum + 
      array:item (array:item fitness-tables cur-node) (sum n-values (interdependencies + 1) [(array:item contents (array:item (array:item input-sets cur-node) ?)) * (2 ^ ?)])
      set cur-node cur-node + 1
    ]
    set fitness (fitness-sum / contents-size)
    ;set fitness ((fitness-sum / contents-size) / base-max-fitness) ^ 8 ; Lazer & Friedman's definition
...
This version (C) Christopher Watts, 2015. See below for terms and conditions of use.
...
Then try using NK fitness. Once some papers get rejected for their less-fit contents, the number of journal publications falls. Adjusting the parameters to get realistic distributions may prove difficult.
